...
func convertConstants() {
	...
	ConstData("EXP2_64", F64(math.Exp2(64)))
	ConstData("EXP2_64_INV", F64(math.Exp2(-64)))
...
func convertUint64ToFloat64AssignAVX2() {
	TEXT("convertUint64ToFloat64AssignAVX2", NOSPLIT, "func(p []uint64, fpOut []float64)")
	Pragma("noescape")
	...
	coeffIn := YMM()
	VMOVDQU(Mem{Base: p, Index: i, Scale: 8}, coeffIn)

	coeffOut := YMM()
	convertInt64ToFloat64(cvtConst, coeffIn, coeffOut)

	VMOVUPD(coeffOut, Mem{Base: fpOut, Index: i, Scale: 8})
...
func convertFloat64ToUint64AssignAVX2() {
	TEXT("convertFloat64ToUint64AssignAVX2", NOSPLIT, "func(fp []float64, pOut []uint64)")
	Pragma("noescape")
	...
	coeffIn := YMM()
	VMOVUPD(Mem{Base: fp, Index: i, Scale: 8}, coeffIn)

	coeffOut := YMM()
	convertFloat64ToInt64(cvtConst, coeffIn, coeffOut)

	VMOVDQU(coeffOut, Mem{Base: pOut, Index: i, Scale: 8})
...
func floatModQInPlaceAVX2() {
	TEXT("floatModQInPlaceAVX2", NOSPLIT, "func(fp []float64)")
	Pragma("noescape")
	...
	Q, QInv := YMM(), YMM()
	VBROADCASTSD(NewDataAddr(NewStaticSymbol("EXP2_64"), 0), Q)
	VBROADCASTSD(NewDataAddr(NewStaticSymbol("EXP2_64_INV"), 0), QInv)
	...
	c := YMM()
	VMOVUPD(Mem{Base: fp, Index: i, Scale: 8}, c)
	...
	VMOVUPD(cOut, Mem{Base: fp, Index: i, Scale: 8})
